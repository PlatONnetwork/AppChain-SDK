...
func (m *Module) ProcessLog(ctx sdk.ConsensusContext, header *coretypes.Header, qc *ctypes.QuorumCert, log *coretypes.Log) error {
	...
	for ; i < uint64(types.CheckpointCommitDis); i++ {
		if m.staking.IsEndOfRound(ctx, block+i) {
			block = block + i + 1
			epoch = epoch + 1
			break
		}
...
func (m *Module) rebuildCheckpoint(ctx sdk.ConsensusContext, qc *ctypes.QuorumCert) (*types.Checkpoint, error) {
	currentValidators, err := m.staking.GetRoundValidator(ctx, qc.BlockNumber)
	if err != nil {
		return nil, err
	}
	currentAccountSet := types.NewAccountSet(currentValidators)
	...
	nextValidators, err := m.staking.GetRoundValidator(ctx, qc.BlockNumber+1)
	if err != nil {
		return nil, err
	}
	nextAccountSet := types.NewAccountSet(nextValidators)
