...
func encodePendingDelegatorRewardKey(delegatorAddr, validatorAddr basecommon.Address) []byte {

	delegatorAddrBytes := delegatorAddr.Bytes()
	validatorAddrBytes := validatorAddr.Bytes()

	keyPrefixSize := len(pendingDelegatorRewardKeyPrefix)
	appendDelegatorAddrSize := keyPrefixSize + len(delegatorAddrBytes)
	size := appendDelegatorAddrSize + len(validatorAddrBytes)

	key := make([]byte, size)

	copy(key[:keyPrefixSize], pendingDelegatorRewardKeyPrefix)
	copy(key[keyPrefixSize:appendDelegatorAddrSize], delegatorAddrBytes)
...
func RemoveEpochDelegationRewardPerShareItem(db sdk.StateDB, addr, validatorAddr basecommon.Address, stakeEpoch, rewardEpoch uint64) error {
	...
	rdblog.Debug("Call RemoveEpochDelegationRewardPerShareItem", "validatorAddr", validatorAddr.Hex(), "stakeEpoch", stakeEpoch, "rewardEpoch", rewardEpoch,
		"item", string(ib), "preIsNil", pre.IsEmpty(), "nextIsNil", next.IsEmpty())

	if pre.IsNotEmpty() {
		pb, _ := json.Marshal(pre)
		rdblog.Debug("Call RemoveEpochDelegationRewardPerShareItem(preIsNotEmpty)", "validatorAddr", validatorAddr.Hex(), "stakeEpoch", stakeEpoch, "rewardEpoch", rewardEpoch,
			"preEpoch", item.PreRewardEpoch, "pre", string(pb))
	}
